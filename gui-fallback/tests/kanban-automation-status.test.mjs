...
assert.match(
  kanbanJs,
  /automation_status_loaded:[\s\S]*automation_review_processor_status:[\s\S]*automation_commit_link_health_ok:/,
...
assert.match(
  kanbanCss,
  /\.kanban-icon-btn--automation::before[\s\S]*\.kanban-automation-grid[\s\S]*#kanban-automation-status-modal/,
  'Automation status icon, metric grid, and modal styles must be present.',
);

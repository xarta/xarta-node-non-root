// voice-timeline-module.js - isolated voice timeline display component.

(function (root, factory) {
  'use strict';
  const api = factory(root);
  if (typeof module === 'object' && module.exports) module.exports = api;
  root.BlueprintsVoiceTimelineModule = api;
})(typeof globalThis !== 'undefined' ? globalThis : window, function (root) {
  'use strict';

  const DEFAULT_TEMPLATE_URL = 'components/voice-timeline-module.html';
  ...
  const CONTRACT = {
    setSnapshot: 'Pass metrics, source, activeState, states, and timeline arrays.',
    clear: 'Return to the zeroed idle presentation.',
    pause: 'Freeze this module only; incoming snapshots are held pending.',
    resume: 'Resume this module and apply the latest pending snapshot.',
  };
  ...
  function emptySnapshot(paused = false) {
    return {
      metrics: [
        { label: 'FSM', value: 'DISABLED' },
        { label: 'Session', value: '--' },
        { label: 'Instance', value: '--' },
        { label: 'Frames', value: '0' },
        { label: 'Debug age', value: '--' },
        { label: 'Level', value: '-80.0 dB' },
      ],
      source: 'No input connected.',
  ...
  function normalizeSnapshot(snapshot, paused = false) {
    ...
    return {
      ...
      timeline: {
        ...fallback.timeline,
        ...timeline,
        endLabel: paused ? 'paused' : text(timeline.endLabel ?? timeline.end_label, fallback.timeline.endLabel),
      },
    };
  }
  ...
  function renderStates(node, states, activeState) {
    ...
    node.innerHTML = states.map(state => (
      `<span class="voice-timeline-module__state${state === activeState ? ' is-active' : ''}">${escapeHtml(state)}</span>`
    )).join('');
  }
  ...
  function loadTemplate(url = DEFAULT_TEMPLATE_URL) {
    if (!templatePromise) {
      const fetchFn = root.fetch;
      templatePromise = fetchFn(url, { cache: 'no-store' })
        .then(response => {
  ...
  function refs(container) {
    return {
      metrics: container.querySelector('[data-vtm-metrics]'),
      source: container.querySelector('[data-vtm-source]'),
      pause: container.querySelector('[data-vtm-pause]'),
      canvas: container.querySelector('[data-vtm-canvas]'),
      states: container.querySelector('[data-vtm-states]'),
    };
  }

  async function create(container, options = {}) {
    if (!container) return null;
    if (container.__blueprintsVoiceTimelineModule) return container.__blueprintsVoiceTimelineModule;
    container.innerHTML = await loadTemplate(options.templateUrl || DEFAULT_TEMPLATE_URL);
    const elements = refs(container);
    ...
    function render() {
      ...
      renderMetrics(elements.metrics, snapshot.metrics);
      if (elements.source) elements.source.textContent = snapshot.source;
      drawTimeline(elements.canvas, snapshot, options);
      renderStates(elements.states, snapshot.states, snapshot.activeState);
      return snapshot;
    }
    ...
    function setPaused(next) {
      ...
      if (elements.pause) {
        ...
        elements.pause.title = state.paused ? 'Resume timeline' : 'Pause timeline';
      }
      return render();
    }
    ...
    const api = {
      ...
      destroy() {
        ...
        root.removeEventListener?.('resize', scheduleRender);
        delete container.__blueprintsVoiceTimelineModule;
      },
    };
    container.__blueprintsVoiceTimelineModule = api;
    render();
    return api;
  }

  function mountDocument(documentRef = root.document, options = {}) {
    const selector = options.selector || '[data-voice-timeline-module-mount]';
    const nodes = Array.from(documentRef?.querySelectorAll?.(selector) || []);
    return Promise.all(nodes.map(node => create(node, options)));
  }

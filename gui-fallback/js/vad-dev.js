...
const VadDevModal = (() => {
  ...
  const PRE_ROLL_FRAMES_DEFAULT = 1;
  ...
  function createPreRollState() {
    return {
      ...
      applied: false,
      reason: 'disabled',
      lookbackFrames: PRE_ROLL_FRAMES_DEFAULT,
    };
  }
  ...
  function clampPreRollFrames(value) {
    const numeric = Number(value);
    const safe = Number.isFinite(numeric) ? numeric : PRE_ROLL_FRAMES_DEFAULT;
    const stepped = Math.round(safe / PRE_ROLL_FRAMES_STEP) * PRE_ROLL_FRAMES_STEP;
    return Math.max(PRE_ROLL_FRAMES_MIN, Math.min(PRE_ROLL_FRAMES_MAX, stepped));
  }

  function preRollFrames() {
    const fromControl = els.preRollFrames ? Number(els.preRollFrames.value) : state.preRollFrames;
    state.preRollFrames = clampPreRollFrames(fromControl);
    return state.preRollFrames;
  }
  ...
  function renderSharedControls() {
    ...
    const savedPreRollFrames = Number(vm?.preRollFrames?.());
    if (Number.isFinite(savedPreRollFrames) && els.preRollFrames) {
      state.preRollFrames = clampPreRollFrames(savedPreRollFrames);
  ...
  function wakeRuntimeInstanceSnapshot(instanceId) {
    ...
    return {
      ...
      vad_payload0_timeout_ms: timeoutMs,
      payload0_timeout_enabled: timeoutMs > 0,
      payload0_timeout_active: !!(instance.awaitingPayload && deadlineAt && timeoutRemainingMs > 0 && !instance.payload0VadActive),
      ...
      payload0_vad_active: !!instance.payload0VadActive,
      ...
      last_payload0_vad_start_at_ms: Number(instance.lastPayload0VadStartAt || 0),
      last_payload0_vad_end_at_ms: Number(instance.lastPayload0VadEndAt || 0),
  ...
  function expireWakeRuntimePayload0(mode, instanceId, reason = 'payload0_timeout') {
    ...
    state.wakeRuntime.instances[instanceId] = instance;
    const now = Date.now();
    if (!instance.awaitingPayload || instance.payload0VadActive) return false;
    ...
    clearWakeRuntimePayload0Timer(instance);
    instance.awaitingPayload = false;
    instance.payload0VadActive = false;
  ...
  function scheduleWakeRuntimePayload0Timeout(mode, instanceId, reason = 'payload0_wait', now = Date.now()) {
    ...
    const vadIsActive = mode !== MODE_MANUAL && !!probe(mode).vad?.speaking;
    if (instance.payload0VadActive || vadIsActive) {
      clearWakeRuntimePayload0Timer(instance);
      if (vadIsActive && !instance.payload0VadActive) {
        instance.payload0VadActive = true;
        instance.lastPayload0VadStartAt = now;
        instance.payload0TimeoutReason = 'vad_active';
        pushAction(mode, 'wakeRuntimePayload0VadHold', {
          instance: instanceId,
          reason,
          timeout_ms: timeoutMs,
          matched_alias: instance.lastMatchedAlias || '',
      ...
      instance.payload0TimeoutReason = 'vad_active';
      dispatchWakeRuntimeChanged('payload0_vad_hold');
      return false;
    }
    if (instance.payload0TimeoutTimer) window.clearTimeout(instance.payload0TimeoutTimer);
  ...
  function noteWakeRuntimePayload0VadActivity(mode, active, reason, now = Date.now()) {
    if (!wakeRuntimeActiveForMode(mode) || wordDetectionPayload0TimeoutMs() <= 0) return false;
    const nextActive = !!active;
    let changed = false;
    WAKE_RUNTIME_INSTANCES.forEach(instanceId => {
      const instance = state.wakeRuntime.instances[instanceId] || createWakeRuntimeInstanceState();
      state.wakeRuntime.instances[instanceId] = instance;
      if (!instance.awaitingPayload) return;
      if (nextActive) {
        if (instance.payload0VadActive) return;
        clearWakeRuntimePayload0Timer(instance);
        instance.payload0VadActive = true;
        instance.lastPayload0VadStartAt = now;
        instance.payload0TimeoutReason = 'vad_active';
      ...
      if (!instance.payload0VadActive) return;
      instance.payload0VadActive = false;
      instance.lastPayload0VadEndAt = now;
      instance.lastUpdatedAt = now;
      pushAction(mode, 'wakeRuntimePayload0VadRelease', {
        instance: instanceId,
        reason,
        timeout_ms: wordDetectionPayload0TimeoutMs(),
        matched_alias: instance.lastMatchedAlias || '',
      });
      scheduleWakeRuntimePayload0Timeout(mode, instanceId, reason || 'vad_end', now);
      changed = true;
    });
    if (changed) {
      dispatchWakeRuntimeChanged(nextActive ? 'payload0_vad_hold' : 'payload0_vad_release');
      poll({ force: true });
    }
    return changed;
  }
  ...
  function armWakeRuntimePayload0(mode, instanceId, finalText, matchedAlias, now = Date.now()) {
    ...
    instance.payload0VadActive = false;
    instance.lastPayload0VadStartAt = 0;
    instance.lastPayload0VadEndAt = 0;
    ...
    if (exactMatches.length) {
      WAKE_RUNTIME_INSTANCES.forEach(instanceId => {
        if (!exactMatches.some(match => match.instanceId === instanceId)) {
          ...
          state.wakeRuntime.instances[instanceId].payload0VadActive = false;
        }
      });
  ...
  function confirmSileroSpeech(mode, reason = 'speech_real_start', now = Date.now()) {
    ...
    if (speechStarted) {
      ...
      noteWakeRuntimePayload0VadActivity(mode, true, 'silero_speech_confirmed', now);
  ...
  function onSileroSpeechEnd(mode, audio) {
    ...
    noteWakeRuntimePayload0VadActivity(mode, false, 'silero_speech_end', now);
    updateSileroStop(mode, now);
  }
  ...
  function resetPreRollSegment(mode, options = {}) {
    ...
    preRoll.lookbackFrames = preRollFrames();
  ...
  function updateVad(mode, features, now = Date.now(), frame = null) {
    ...
    if (vad.candidateActive && !vad.candidateConfirmed && candidatePersisted && (strongFrame || vad.candidateFrames >= VAD_TEST_CONFIRM_FRAMES)) {
      ...
      if (speechStarted) {
        ...
        noteWakeRuntimePayload0VadActivity(mode, true, 'energy_vad_confirmed', now);
      ...
      noteWakeRuntimePayload0VadActivity(mode, false, 'energy_vad_hangover_end', now);
    }
  ...
  function preparePreRollForRecording(mode, options = {}) {
    ...
    const lookbackFrames = preRollFrames();
    preRoll.lookbackFrames = lookbackFrames;
    let startFrameId = detectionFrameId - lookbackFrames;
    let fallbackReason = '';
    if (!oldest || !latest) {
    ...
    if (startFrameId < oldest) {
      startFrameId = oldest;
      if (detectionFrameId <= oldest) {
        fallbackReason = 'previous_frame_unavailable';
      } else {
        fallbackReason = oldest <= 1 ? 'previous_frames_limited' : 'previous_frame_rolled_out';
      }
    }
    ...
    pushAction(mode, 'autoPreRollSelect', {
      ...
      num_pre_roll_frames: lookbackFrames,
  ...
  function preRollSnapshot(mode) {
    const preRoll = probe(mode).preRoll || createPreRollState();
    const auto = autoPreRollStatus(mode);
    const configuredFrames = preRollFrames();
    const segmentLookbackFrames = Number(preRoll.lookbackFrames);
    const lookbackFrames = preRoll.applied && Number.isFinite(segmentLookbackFrames)
      ? clampPreRollFrames(segmentLookbackFrames)
      : configuredFrames;
    return {
      ...
      pre_roll_frames: lookbackFrames,
      num_pre_roll_frames: lookbackFrames,
      pre_roll_lookback_frames: lookbackFrames,
      pre_roll_requested_frames: lookbackFrames,
      pre_roll_configured_frames: configuredFrames,
  ...
  async function startRecording(mode, options = {}) {
    ...
    try {
      ...
      pushAction(mode, mode === MODE_MANUAL ? 'manualRecordStart' : (mode === MODE_REARM ? 'rearmProbeRecordStart' : 'vadProbeRecordStart'), {
        ...
        pre_roll_frames: preRoll.pre_roll_lookback_frames,
        num_pre_roll_frames: preRoll.pre_roll_lookback_frames,
        pre_roll_selected_frames: queued.length,
        pre_roll_sent_frames: queued.length,
        pre_roll_lookback_frames: preRoll.pre_roll_lookback_frames,
  ...
  function actionColor(mode, action) {
    ...
    if (type === 'vadRecordMode' || type === 'vadDetectorMode' || type === 'vadInterruptTtsMode' || type === 'wordDetectionInterruptTtsMode' || type === 'wordDetectionPayload0TimeoutMode' || type === 'wordDetectionPayload0VadHold' || type === 'wakeRuntimePayload0VadHold' || type === 'vadTestMode' || type === 'rearmTestMode' || type === 'vadCandidateStart' || type === 'sileroLoad' || type === 'sileroCandidateStart' || type === 'wakeRuntimeSenseWordMatched') return '#38bdf8';
  ...
  function actionLabel(action) {
    ...
    if (action.type === 'wakeRuntimePayload0VadHold') return `Wake ${action.instance || ''} payload0 hold`.trim();
    if (action.type === 'wakeRuntimePayload0VadRelease') return `Wake ${action.instance || ''} payload0 release`.trim();
  ...
  async function setPreRollFrames(value, options = {}) {
    const frames = clampPreRollFrames(value);
    state.preRollFrames = frames;
    if (els.preRollFrames) els.preRollFrames.value = String(frames);
    MODES.forEach(mode => {
      const preRoll = probe(mode).preRoll || createPreRollState();
      probe(mode).preRoll = preRoll;
      preRoll.lookbackFrames = frames;
    });
    renderRangeLabels();
    const mode = currentMode();
    if (mode !== MODE_MANUAL) {
      pushAction(mode, 'preRollFramesMode', {
        pre_roll_frames: frames,
        num_pre_roll_frames: frames,
        reason: options.reason || 'set_pre_roll_frames',
      });
    }
    try {
      await voiceMode()?.savePreRollFrames?.(frames);
      status(options.status || 'NUM PRE-ROLL saved.');
    } catch (error) {
      status(`NUM PRE-ROLL save failed: ${error.message || error}`);
    }
    renderSharedControls();
    poll({ force: true });
    return frames;
  ...
  async function runSettingsCommand(action, payload) {
    ...
    if (action === 'set_pre_roll_frames' || action === 'set_num_pre_roll' || action === 'set_num_pre_roll_frames') {
      const value = payloadNumber(payload, 'pre_roll_frames', 'num_pre_roll_frames', 'num_pre_roll', 'frames');
  ...
  function bind() {
    ...
    els.preRollFrames?.addEventListener('change', () => {
      void setPreRollFrames(els.preRollFrames.value, { reason: 'ui_slider' });
    });

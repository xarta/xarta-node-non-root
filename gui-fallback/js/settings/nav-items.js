...
function _niAssetActionCell(item, options) {
    ...
    const menuActions = `${options.includeUpload ? `<button type="button" class="table-icon-btn table-icon-btn--upload" data-ni-upload="${_esc(options.assetType)}" data-item-id="${_esc(item.item_id)}" title="${_esc(options.uploadTitle)}" aria-label="${_esc(options.uploadTitle)}"></button>` : ''}${options.includeBrowse ? `<button type="button" class="table-icon-btn table-icon-btn--browse" data-ni-browse="${_esc(options.assetType)}" data-item-id="${_esc(item.item_id)}" title="${_esc(options.browseTitle)}" aria-label="${_esc(options.browseTitle)}"></button>` : ''}${options.includePlay && path ? `<button type="button" class="table-icon-btn table-icon-btn--play-toggle" data-ni-play="${_esc(path)}" data-updated="${_esc(item.updated_at || '')}" title="${_esc(options.playTitle)}" aria-label="${_esc(options.playTitle)}" aria-pressed="false">▶</button>` : ''}${options.includeClear && path ? `<button type="button" class="table-icon-btn table-icon-btn--clear-asset" data-ni-clear="${_esc(options.pathField)}" data-item-id="${_esc(item.item_id)}" title="${_esc(options.clearTitle)}" aria-label="${_esc(options.clearTitle)}"></button>` : ''}`;
    const menuPath = `<div class="table-asset-menu-path" title="${_esc(path || 'No asset assigned')}">${_esc(path || 'No asset assigned')}</div>`;
    const collapsedActions = `<span class="table-asset-menu-anchor"><button type="button" class="table-icon-btn table-icon-btn--scarab" title="${_esc(options.menuTitle)}" aria-label="${_esc(options.menuTitle)}" aria-expanded="${_niOpenAssetMenu === menuKey ? 'true' : 'false'}" data-ni-asset-menu-trigger="${_esc(menuKey)}"></button>${_niOpenAssetMenu === menuKey ? `<span class="table-asset-menu" data-ni-asset-menu="${_esc(menuKey)}">${menuPath}<span class="table-asset-menu-actions">${menuActions}</span></span>` : ''}</span>`;
    return `<td class="${_esc(options.cellClass)}"><div class="ni-asset-cell${shouldCollapse ? ' is-collapsed' : ''}">${pathHtml}<div class="ni-asset-btns"><input type="file" class="ni-file-input" style="display:none" data-item-id="${_esc(item.item_id)}" data-asset-type="${_esc(options.assetType)}" accept="${_esc(options.accept)}">${shouldCollapse ? collapsedActions : inlineActions}</div></div></td>`;
}

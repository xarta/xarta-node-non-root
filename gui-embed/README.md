...
`blueprints-auth.js` handles TOTP authentication. On any site that doesn't already define `window.apiFetch`, it installs one that reads the API secret from `localStorage` (key `blueprints_api_secret`) and derives a time-based token for each request. On a 401 it opens the embedded API-key modal, lets the user save the secret, and retries once. The main Blueprints GUI defines its own `apiFetch` — if both scripts are present, `blueprints-auth.js` detects this and does nothing, so there is no conflict.

The standalone/embed auth flow now uses an embedded modal dialog rather than a browser `prompt()`. The selector also exposes two API-key-related actions when configured:
- `api-key` — opens the normal key editor
- `api-key-test` — force-opens the embedded modal in its auth-failed state so the fallback UX can be tested without poisoning localStorage first
